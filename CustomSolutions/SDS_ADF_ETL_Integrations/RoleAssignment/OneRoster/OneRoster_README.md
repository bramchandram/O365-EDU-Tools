...
3) Load the OR_to_SDS_ADF_Template.json file by clicking "Load file" in the template editor and selecting the appropriate file to upload.  Create a resource group where to place the resources if not already done. (suggested name rg-OneRosterAPItoSDS). Note: The optional parameters on the confirmation screen can be filled in later during the ADF setup.
...
5) If you want to use SFTP, modify the storage account access to enable managed
   identity adf-OneRostertoSDS (ADF instance) to toggle SFTP. (“Storage Account Contributor” role). The incoming IP address should also be added to firewall in the storage account.
